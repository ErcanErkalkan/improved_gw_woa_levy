...
def plot_mean_convergence(results, analysis):
    ...
    for algo,data in results.items():
        if analysis[algo]['success_rate']>0:
            maxlen = max(len(h) for h in data['histories']) if data['histories'] else 0
            if maxlen == 0: 
                continue
            padded = [h+ [h[-1]]*(maxlen-len(h)) for h in data['histories'] if len(h)>0]
            if not padded: 
                continue
            m = np.mean(padded,axis=0); s=np.std(padded,axis=0, ddof=1)
            plt.plot(m,label=algo)
            plt.fill_between(range(maxlen),m-s,m+s,alpha=0.2)
    plt.title('Avg Convergence (100 trials)')
    plt.xlabel('Iter');plt.ylabel('Cost');plt.legend();plt.grid();plt.savefig('average_convergence.png')
...
# ---------------------- CSV Utilities ----------------------
def write_results_all_csv(results, seeds, out_path="outputs/results_all.csv"):
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    # results[name]['costs'][run_idx] = min cost (may be inf if failed)
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["method","run","seed","cost"])
        # Her method için aynı uzunlukta cost listesi varsayımı
        for method, pack in results.items():
            costs = pack['costs']
            for run_idx, cost in enumerate(costs):
                seed = seeds[run_idx] if run_idx < len(seeds) else run_idx
                w.writerow([method, run_idx+1, seed, cost])

def write_analysis_summary_csv(results, out_path="outputs/analysis_summary.csv"):
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    rows=[]
    for a,r in results.items():
        arr = np.array(r['costs'], dtype=float)
        ...
        if n_ok > 0:
            ...
            mean = std = mn = mx = med = q1 = q3 = iqr = float("nan")
            succ_rate = 0.0
        rows.append({
            "method": a, "n_runs": n, "n_success": n_ok, "success_rate": succ_rate,
            "mean": mean, "std": std, "min": mn, "max": mx,
            "median": med, "q1": q1, "q3": q3, "iqr": iqr
        })
    # method adına göre sıralayabilirsiniz (opsiyonel)
    with open(out_path, "w", newline="", encoding="utf-8") as f:
...
if __name__=='__main__':
    ...
    # İsimlendirmeyi EGWWOA olarak düzeltelim (CSV/statistik uyumu için)
    algos = {
        'GA':'run_ga','GWO':'run_gwo','EGWWOA':'run_gwwoa','WOA':'run_woa',
    ...
    for t in range(N_TRIALS):
        ...
        for name,fn in algos.items():
            try:
                ...
                # Bu denemedeki en iyi (en düşük) maliyet
                best_cost = np.min(h) if (h is not None and len(h)>0) else np.inf
                results[name]['costs'].append(best_cost)
            except Exception as e:
                # Hata durumunda başarısız koşuyu işaretle
                results[name]['solutions'].append(None)
                results[name]['histories'].append([])
                results[name]['costs'].append(np.inf)

    # Analiz sözlüğü (konsol çıktısı)
    analysis={a:{
        'mean':np.nanmean([c for c in r['costs'] if np.isfinite(c)]) if any(np.isfinite(r['costs'])) else np.nan,
        'std': np.nanstd([c for c in r['costs'] if np.isfinite(c)], ddof=1) if sum(np.isfinite(r['costs']))>1 else np.nan,
        'min':np.nanmin([c for c in r['costs'] if np.isfinite(c)]) if any(np.isfinite(r['costs'])) else np.nan,
        'max':np.nanmax([c for c in r['costs'] if np.isfinite(c)]) if any(np.isfinite(r['costs'])) else np.nan,
        'success_rate':float(np.isfinite(r['costs']).mean())
    } for a,r in results.items()}

    print('Final Performance Summary:')
    for a,v in analysis.items():
        print(f"{a}: mean={v['mean']:.1f}, std={v['std']:.1f}, min={v['min']:.1f}, max={v['max']:.1f}, success={v['success_rate']:.0%}")

    # Grafikleri üret
    plot_mean_convergence(results,analysis)
    groups={'Group1':['GA','GWO','EGWWOA','WOA'],
    ...
    # ---- CSV çıktıları ----
    write_results_all_csv(results, seeds_used, out_path="outputs/results_all.csv")
    write_analysis_summary_csv(results, out_path="outputs/analysis_summary.csv")
    print("CSV dosyaları yazıldı: outputs/results_all.csv, outputs/analysis_summary.csv")
